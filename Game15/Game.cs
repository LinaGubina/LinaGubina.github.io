...
namespace Game15
{
    class Game
    {
        ...
        public void start()
        {
            ...
            space_x = size - 1;
            space_y = size - 1;
            map[space_x, space_y] = 0; //нумерация последней ячейки
        }
        ...
        //возвращение цифры, число которой стоит в нужном месте
        public int get_number(int position)
        {
            int x, y;
            position_to_coords(position, out x, out y);
            //проверка, чтобы не было переполнения массива
        ...
        //вспомогательные функции (из координаты в позицию и наоборот)
